...
#include "gc/jstgc/jstgcMonitoringSupport.hpp"
...
class JstgcSpaceCounters: public CHeapObj<mtGC> {
...
public:
  JstgcSpaceCounters(const char* name,
                 int ordinal,
                 size_t max_size,
...
class JstgcGenerationCounters : public GenerationCounters {
private:
  JstgcHeap* _heap;
public:
  JstgcGenerationCounters(JstgcHeap* heap) :
...
JstgcMonitoringSupport::JstgcMonitoringSupport(JstgcHeap* heap) {
  _heap_counters  = new JstgcGenerationCounters(heap);
  _space_counters = new JstgcSpaceCounters("Heap", 0, heap->max_capacity(), 0, _heap_counters);
}

void JstgcMonitoringSupport::update_counters() {
  MemoryService::track_memory_usage();

  if (UsePerfData) {
    EpsilonHeap* heap = JstgcHeap::heap();
