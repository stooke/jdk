...
#include "gc/jstgc/jstgcInitLogger.hpp"
...
void JstgcInitLogger::print_gc_specific() {
  if (UseTLAB) {
    size_t max_tlab = JstgcHeap::heap()->max_tlab_size() * HeapWordSize;
    log_info(gc, init)("TLAB Size Max: " EXACTFMT, EXACTFMTARGS(max_tlab));
    if (JstgcElasticTLAB) {
      log_info(gc, init)("TLAB Size Elasticity: %.2fx", JstgcTLABElasticity);
    }
    if (JstgcElasticTLABDecay) {
      log_info(gc, init)("TLAB Size Decay Time: %zums", JstgcTLABDecayTime);
    }
  } else {
...
void JstgcInitLogger::print() {
  JstgcInitLogger init_log;
  init_log.print_all();
}
